			...
			if ( ! tri.IsNull() ) {
				...
				for( int i = 1; i <= nodes.Length(); ++ i ) {
					...
					if ( calculate_normals ) {
						...
						_normals.push_back((double)normal.X());
						_normals.push_back((double)normal.Y());
						_normals.push_back((double)normal.Z());
					}
				}
		...
		, _guid(guid)
{
	...
	for( int i = 1; i < 5; ++ i )
		for ( int j = 1; j < 4; ++ j )
			_matrix.push_back((double)trsf.Value(j,i));
}
...
const std::vector<double>& IfcGeomObjects::IfcRepresentationTriangulation::verts() const { return _verts; }
...
const std::vector<double>& IfcGeomObjects::IfcRepresentationTriangulation::normals() const { return _normals; }
...
const std::vector<double>& IfcGeomObjects::IfcObject::matrix() const { return _matrix; }

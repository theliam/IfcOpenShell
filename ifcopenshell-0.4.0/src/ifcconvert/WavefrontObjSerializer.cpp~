...
void WaveFrontOBJSerializer::writeTesselated(const IfcGeomObjects::IfcGeomObject* o) {
	...
	for ( std::vector<double>::const_iterator it = mesh.verts().begin(); it != mesh.verts().end(); ) {
		...
		const double y = *(it++);
		const double z = *(it++);
		obj_stream << "v " << std::fixed << std::setprecision(8) << x << " " << std::fixed << std::setprecision(8) << y << " " << std::fixed << std::setprecision(8) << z << std::endl;
	}
	for ( std::vector<double>::const_iterator it = mesh.normals().begin(); it != mesh.normals().end(); ) {
		const double x = *(it++);
		const double y = *(it++);
		const double z = *(it++);
		obj_stream << "vn " << std::fixed << std::setprecision(8) << x << " " << std::fixed << std::setprecision(8) << y << " " << std::fixed << std::setprecision(8) << z << std::endl;
	}
